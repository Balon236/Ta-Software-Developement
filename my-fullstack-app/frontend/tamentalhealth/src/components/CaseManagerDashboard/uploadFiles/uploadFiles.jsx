...
const UploadFiles = () => {
  ...
  const handleExport = () => {
    const csvContent =
      ...
      ["File Name,File Type,Uploaded By,Created At"]
        .concat(
          filteredData.map(
            (item) =>
              `${item.fileName},${item.fileType},${item.uploadBy},${item.createdAt}`
          )
        )
        .join("\n");
    ...
    link.setAttribute("download", "uploaded_files_data.csv");
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black/50 bg-opacity-50 flex justify-center items-center z-[1000]">
          <div className="bg-white p-6 rounded-lg max-w-md w-full">
            <h3 className="text-lg font-medium mb-4">Edit File Details</h3>
