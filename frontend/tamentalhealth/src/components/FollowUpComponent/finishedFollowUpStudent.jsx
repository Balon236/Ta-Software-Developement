  ...
  const fetchCompletedFollowups = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/clients/followup/completed",
        {
          params: { manager_id: 1 },
        }
      );
      ...
      const formatted = response.data.data.map((item) => ({
        ...
        completedDate: item.completion_date || "N/A",
        follomUpSummary: item.summary || "N/A",
        finalOutcome: item.final_outcome || 0,
      }));
