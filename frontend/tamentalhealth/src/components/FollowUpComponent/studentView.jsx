  ...
  const [selectedIds, setSelectedIds] = useState([]);
   ...
   useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/clients?manager_id=1');
        ...
        const formatted = clients.map(client => ({
          ...
          code: client.code ?? "-",
          class: client.classroom?.name ?? "N/A",
          screeningResult: client.screened === "yes" ? "View result link" : "Not screened",
          consultant: client.consultant?.name ?? "N/A",
          consResult: "View result link", // Replace with actual logic or URL if available
        }));
