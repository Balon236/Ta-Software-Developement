...
const ViewAgenda = () => {
  ...
  useEffect(() => {
    const fetchReminders = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/reminder/due-today");

        // Fallback in case any task is missing data
        const formattedTasks = response.data.map((t) => ({
          id: t.id,
          topic: t.topic || "Untitled Task",
          description: t.description || "No description",
          duration: t.duration || "Unknown",
        })); 

        setTasks(formattedTasks);
        setUpdatedTasks(formattedTasks);
      } catch (error) {
  ...
  onClick={async () => {
    try {
      ...
      await axios.post("http://localhost:8000/api/reminder/done", {
        tasks: completed,
      });
