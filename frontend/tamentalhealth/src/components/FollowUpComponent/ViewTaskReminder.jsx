...
function TaskReminder() {
  ...
  */]);
  ...
  const [selectedIds, setSelectedIds] = useState([]);
   ...
   useEffect(() => {
    axios
      .get("http://localhost:8000/api/reminder/all?manager_id=1") // adjust URL to match your route
      .then((response) => {
        const transformed = response.data.map((item, index) => ({
          id: index + 1, // Add ID if not provided by backend
          completedDate: new Date(item.due_date).toLocaleString(), // Format date
          reminderType:
            item.type === "followup" ? "Follow-up" : "Scheduled Task",
        ...
        <tbody>
          {paginatedData.map((item) => (
            <tr key={item.id}>
              ...
              <td className="fullName-column">{item.name}</td>
              ...
              <td className="school-name-column">{item.status}</td>
