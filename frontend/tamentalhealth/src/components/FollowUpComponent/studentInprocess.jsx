  ...
  */]);
  ...
  useEffect(() => {
    const fetchFollowUps = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/clients/filter/followup-status', {
          params: {
            status: 'in_progress',
            manager_id: 1,
          },
        ...
        const formattedData = response.data.data.map((item, index) => ({
          id: index + 1, // or use a real unique identifier if available
          studentName: item.name || "N/A",
          code: item.code || "N/A",
          class: item.class || "N/A",
          screeningResult: "here is the link", // replace with actual link if available
          consultant: item.consultant || "N/A",
          consResult: "here is the link", // replace with actual link if available
        }));
  ...
  return (
    <div className="bg-white rounded-lg border-2 border-blue-100 p-5 mx-auto shadow-sm">
