 ...
 useEffect(() => {
  ...
  useEffect(() => {
    if (selectedSchoolId) {
      ...
      axios
        .get(`http://localhost:8000/api/classrooms/by-school/${selectedSchoolId}`)
    ...
    <div className="bg-white p-4 md:p-6 ">
      ...
      <div className="flex flex-col gap-3 md:gap-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
            ...
            {/* Select School (used only to fetch classes) */}
      ...
      <div className="flex flex-col gap-1 col-span-1">
        ...
        <select
          name="classId"
          value={formData.demographic.classId}
...
const OnboardingForm = ({ onSubmit }) => {
  ...
  const handleSubmit = async () => {
      ...
      try {
        const response = await axios.post('http://127.0.0.1:8000/api/screening-responses/initial-submit', {
          demographic: formData.demographic,
          parental: formData.parental,
          academic: formData.academic,
          physical: formData.physical,
          final: formData.final,
        });
    
        const screenRecordId = response.data.screen_record_id;
        localStorage.setItem('screen_record_id', screenRecordId); 
        alert('Initial form submitted! Use this ID for ACEs: ' + screenRecordId);
      } catch (err) {
        console.error('Initial submission failed', err);
        alert('Initial submission failed.');
      }
