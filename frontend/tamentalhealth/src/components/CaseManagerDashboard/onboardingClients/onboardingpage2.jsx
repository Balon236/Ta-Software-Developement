...
const OnboardingPage2 = () => {
  ...
  const handleSubmit = async () => {
    ...
    const API_URL = 'http://127.0.0.1:8000/api/screening-responses/checklist-submit';
    const screenRecordId = localStorage.getItem('screen_record_id');
  

  if (!screenRecordId) {
    alert('No screen record ID found. Please complete the initial form first.');
    return;
  }

  
      // Loop through each key and post the answer
      const responses = Object.entries(formData).map(([key, value], index) => ({
        question_number: 54 + index, // Assuming ACE questions start at 54
        answer: value.checked ? 'yes' : 'no',
        comment: value.comment || null,
      }));
    
      try {
        await axios.post('http://127.0.0.1:8000/api/screening-responses/checklist-submit', {
          screen_record_id: parseInt(screenRecordId),
          responses,
        });
    
        localStorage.removeItem('screen_record_id');
        alert('ACE checklist submitted successfully!');
        navigate('/onboarding'); // Navigate to the next part of the onboarding process
      } catch (error) {
        console.error('Submission failed:', error);
        alert('Failed to submit ACE checklist.');
      }
