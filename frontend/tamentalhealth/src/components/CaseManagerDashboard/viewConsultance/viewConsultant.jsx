  ...
  */]);
  ...
  useEffect(() => {
    const fetchConsultants = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/consultant/all");
        ...
        const formatted = response.data.map((consultant, index) => ({
          id: consultant.id || index + 1,
          avater: profie,
          fullName: consultant.name || "Unknown",
          email: consultant.email || "not.available@example.com", // if email is included in User model
          phoneNumber: consultant.contact || "No contact",
          role: consultant.role || "N/A",
          gender: consultant.gender || "N/A",
          specialty: consultant.gender || "N/A", // Replace with actual specialty field if exists
        }));
