  ...
  */]);
  ...
  useEffect(() => {
    const fetchReferrals = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/followup/filter", {
          params: {
        ...
        if (!Array.isArray(response.data)) {
          console.warn("Invalid referral response format.");
          return;
        }
  
        const formatted = response.data.map((referral, index) => ({
          id: referral.id || index + 1,
          avater: profie,
          studentName: referral.student_name || "Unknown",
          code: referral.code || "N/A",
          class: referral.class || "N/A",
          screeningResult: referral.screening_result_link || "N/A",
          consultant: referral.consultant_name || "N/A",
          consResult: referral.consultant_result_link || "N/A"
        }));
