...
function ViewAllTask() {
    ...
    const fetchTasks = async () => {
      try {
        ...
        const mapped = tasks.map((task, index) => ({
          ...
          taskExplanation: task?.explanation ?? "No explanation",
          createdAt: task?.created_at
            ? new Date(task.created_at).toLocaleDateString("en-GB") // Format: dd-mm-yyyy
            : "Unknown date",
        }));
  ...
  const handleEditClick = (item) => {
    ...
    setEditFormData({
      ...
      taskExplanation: item.taskExplanation,
  ...
  const handleExport = () => {
    const csvContent =
      ...
      ["Task Name,Description,Explanation,Created At"]
        .concat(
          filteredData.map(
            (item) =>
              `${item.taskName},${item.taskdescription},${item.taskExplanation},${item.createdAt}`
          )
        )
        .join("\n");
    ...
    link.setAttribute("download", "all_tasks_data.csv");
  ...
  return (
    <div className="bg-white rounded-lg border-2 border-blue-100/20 p-5 mx-auto shadow-sm">
      <h2 className="text-2xl text-gray-800 mb-5 font-medium">All Tasks</h2>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    value={editFormData.taskExplanation}
                    onChange={handleEditFormChange}
                    className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                    required
                    rows="3"
