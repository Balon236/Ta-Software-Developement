  ...
  */
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/task/assigned");
        ...
        const tasks = response.data.map((task, index) => ({
          id: index + 1,
          taskName: task.name,
          assignTo: task.manager,
          status: task.status,
          createdAt: new Date(task.created_at).toLocaleDateString("en-GB"), // Format: DD/MM/YYYY
        }));
