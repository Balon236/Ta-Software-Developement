...
const ViewMildSeverity = () => {
  ...
  */]);
  ...
  useEffect(() => {
    const fetchMildClients = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/clients/filter/mild");
  ...
  const formattedClients = response.data.data.map((client, index) => ({
  id: index + 1,
  avater: profie,
  fullName: client.name || `${client.first_name ?? ""} ${client.last_name ?? ""}`.trim(),
  code: `#${client.code || Math.floor(10000 + Math.random() * 90000)}`,
  school: client.school || "Unknown School", // Now just client.school
  results: client.results || "N/A",
  gender: client.gender || "N/A",
  referLink: `/case-manager-dashboard/sevierity-form/${client.id}`, // Example: use client.id for a real link
}));
  
        setData(formattedClients);
      } catch (error) {
        console.error("Error fetching clients by severity level:", error.message);
      }
    };
  ...
  const handleExport = () => {
    const csvContent =
      ...
      ["Full Name,Code,School,Results,Gender,Refer Link"]
        .concat(
          filteredData.map(
            (item) =>
    ...
    link.setAttribute("download", "mild_severity_data.csv");
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black/50 bg-opacity-50 flex justify-center items-center z-[1000]">
          <div className="bg-white p-6 rounded-lg max-w-md w-full">
            <h3 className="text-lg font-medium mb-4">Edit Mild Severity</h3>
...
export default ViewMildSeverity;
