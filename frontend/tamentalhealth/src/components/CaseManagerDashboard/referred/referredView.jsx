  ...
  useEffect(() => {
    const fetchReferrals = async () => {
      try {
        ...
        const mapped = referrals.map((ref, index) => ({
          id: index + 1,
          avatar: profile, // Default profile image
          fullName: ref?.client_name ?? "N/A",
          referredCode: `#${String(ref?.referral_id ?? 0).padStart(5, '0')}`,
          school: ref?.school ?? "N/A",
          class: ref?.class ?? "N/A",
          at: ref?.date_of_referral ?? "Unknown",
          referredTo: ref?.consultant_name ?? "Unassigned",
        }));
