...
const CaseManager = () => {
  ...
  */
  ...
  useEffect(() => {
    axios.get('http://127.0.0.1:8000/api/case-managers')
      .then(response => {
        ...
        if (
          ...
          const transformedData = response.data.data.map(manager => ({
            id: manager?.id || null,
            avater: profie,
            fullName: manager?.name || "No Name",
            email: manager?.email || "No Email",
            phoneNumber: manager?.contact || "No Contact",
            birthDay: manager?.dob || "No DOB",
            gender: manager?.gender || "No Gender",
            specialty:
              Array.isArray(manager?.specialties) && manager.specialties.length > 0
                ? manager.specialties[0].name
                : "No Specialty",
          }));
